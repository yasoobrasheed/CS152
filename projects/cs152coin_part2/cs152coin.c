...
void deanon_preprocess(blockchain_t* bc)
{
    ...
    // Remember the front of the blockchain so that it frees correctly
    block_t* front = bc->blocks;
    while (block) {
        trans_t* trans = block->trans;
        while (trans) {
            add_associations(trans->inputs, trans->num_inputs, d);
            trans = trans->next;
        }
        block = block->next;
    }
    bc->deanon = d;
    // Reset to the front of the blockchain so that it frees correctly
    bc->blocks = front;
}
...
double cluster_balance(cluster_t* cluster, blockchain_t* bc)
{
    ...
    double temp_bal = 0;
    while (cluster) {
        get_balance(cluster->addr, &temp_bal, bc->addr_bal);
        bal += temp_bal;
        cluster = cluster->next;
    }
    return bal;
